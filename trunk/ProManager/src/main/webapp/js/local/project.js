...
$(document).ready(function() {
    ...
    function displayProjects(projects) {
        ...
        for ( var i = 0 ; i < projects.length; i++ ) {
            ...
            "<a href='javascript:void(0);' class='projectUsers' id='projectUsers"+projects[i].id +"'>Members</a>"];
    ...
    $('#projectTable td').livequery( function () {
        $(this).not('.action').click(function () {
            var _projectName = $(this).parent().children()[0].firstChild.data;
            viewed = getProject(_projectName);
            $('#newProjectContainer').modal();
    ...
    function getProject (projectName) {
        for ( var x = 0; x < projectCache.length; x++ ) {
            if ( projectCache[x].name == projectName) {
                return projectCache[x];
            }
        }
    }
